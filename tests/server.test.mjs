...
test('shared app protects every route and keeps export computation in the browser', async t => {
  ...
  assert.match(browserHtml, /id="exportLayerScope"/);
  assert.match(browserHtml, /id="motionScopeValue"/);
  ...
  const appScript = await (await fetch(`${base}/app.js`, { headers })).text();
  assert.match(appScript, /selectedLayerIds/);
  ...
  assert.match(appScript, /function previewFrames\(\)/);
  assert.match(appScript, /function layerFrameAt\(layer, index\)/);
  assert.match(appScript, /motionConfig\(\)\.loop !== 'once'/);
  assert.match(appScript, /layer\.fps = state\.fps/);
  assert.match(appScript, /const fps = activeLayer\(\)\?\.fps \|\| state\.fps/);
  assert.match(appScript, /function copySelectedFrames\(\)/);
