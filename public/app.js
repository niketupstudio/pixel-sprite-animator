...
function compositionEntries() {
  ...
  layers.filter(layer => layer.sheetId && layer.visible && Array.isArray(layer.frameIds) && layer.frameIds.length).forEach(layer => {
    ...
    const frame = ids[state.playIndex % ids.length]; if (frame && frameVisible(frame, layer)) entries.push({ frame, layer });
  });
  if (!entries.length && frames.length) { const frame = frames[clamp(state.playIndex, 0, frames.length - 1)]; if (frameVisible(frame)) entries.push({ frame, layer: null }); }
  return entries;
}
function defaultMotion() { return { mode: 'static', loop: 'static', startX: 0, startY: 0, endX: 0, endY: 0, height: 0, padding: 10, pixelSnap: true }; }
function activeAnimation() { const anim = state.animations[state.activeAnimation]; if (anim && !anim.motion) anim.motion = defaultMotion(); if (anim) ensureLayers(anim); return anim; }
function motionConfig() { return activeAnimation()?.motion || defaultMotion(); }
function averageCharacterWidth() { const frames = activeFrames(); return frames.length ? Math.max(1, Math.round(frames.reduce((sum, f) => sum + f.w, 0) / frames.length)) : 1; }
function averageCharacterHeight() { const frames = activeFrames(); return frames.length ? Math.max(1, Math.round(frames.reduce((sum, f) => sum + f.h, 0) / frames.length)) : 1; }
function motionPixelHeight(motion = motionConfig()) { return Math.max(0, Number(motion.height) || 0) * averageCharacterHeight(); }
function motionPosition(t, motion = motionConfig()) { if (!motion || motion.mode === 'static') return { x: 0, y: 0 }; const p = Math.max(0, Math.min(1, t)); const x = motion.startX + (motion.endX - motion.startX) * p; let y = motion.startY + (motion.endY - motion.startY) * p; if (motion.mode === 'jump') y -= 4 * motionPixelHeight(motion) * p * (1 - p); if (motion.mode === 'custom') { const cx = Number.isFinite(motion.controlX) ? motion.controlX : (motion.startX + motion.endX) / 2; const cy = Number.isFinite(motion.controlY) ? motion.controlY : (motion.startY + motion.endY) / 2; return { x: (1 - p) * (1 - p) * motion.startX + 2 * (1 - p) * p * cx + p * p * motion.endX, y: (1 - p) * (1 - p) * motion.startY + 2 * (1 - p) * p * cy + p * p * motion.endY }; } return { x, y }; }
function simpleMotionAmount() { return state.motionEnabled ? Math.max(0, Number(state.motionAmount) || 0) : 0; }
function simpleMotionOffset(t) { const amount = simpleMotionAmount(); if (!amount) return { x: 0, y: 0 }; const phase = Math.max(0, Math.min(1, t)) * Math.PI * 2; return { x: Math.round(Math.sin(phase) * amount), y: Math.round(Math.sin(phase * 2) * Math.min(1, amount * 0.5)) }; }
function previewMotionPosition(t) { const base = motionPosition(t); const nudge = simpleMotionOffset(t); return { x: base.x + nudge.x, y: base.y + nudge.y }; }
function previewMotionT() { const frames = activeFrames(); if (!frames.length) return 0; return frames.length > 1 ? clamp(state.motionProgress, 0, 1) : 1; }
...
function addFrame(s = state.selection, silent = false) {
  ...
  const frame = createFrame(s); state.frames.push(frame); state.animations[state.activeAnimation].frameIds.push(frame.id); const layer = sheetLayer(frame.sourceSheetId); if (layer) layer.frameIds = [...(layer.frameIds || []), frame.id]; state.selectedFrame = state.frames.length - 1; renderLayers(); renderFrames(); renderAnimations(); drawPreview(); if (!silent) toast(`已添加单帧，自动锚点：(${frame.anchorX}, ${frame.anchorY})`);
}
...
function selectFrameForEdit(globalIndex, additive = false) { const frame = state.frames[globalIndex]; if (!frame) return; if (additive) { if (state.selectedFrameIds.has(frame.id)) state.selectedFrameIds.delete(frame.id); else state.selectedFrameIds.add(frame.id); } else { state.selectedFrameIds = new Set([frame.id]); } state.selectedFrame = globalIndex; renderTimelineGrid(); renderFrames(); drawPreview(); syncSelectionControls(); }
...
function renderTimelineGrid() {
  ...
  const frames = activeFrames(); const layers = ensureLayers(); grid.innerHTML = ''; grid.style.setProperty('--timeline-columns', frames.length || 1); if (!frames.length) return;
...
function renderLayers() {
  ...
  layers.forEach((layer, index) => {
    const row = document.createElement('div'); row.className = `layer-row layer-${layer.mode}${layer.id === state.activeLayerId ? ' active' : ''}`; row.dataset.id = layer.id;
    row.addEventListener('click', event => { if (event.target.closest('button,input,select')) return; state.activeLayerId = state.activeLayerId === layer.id ? null : layer.id; state.selectedFrameIds.clear(); renderLayers(); renderFrames(); drawPreview(); });
...
function finishFrameSelection(event) { const drag = state.frameSelectionDrag; if (!drag) return; const point = frameListPoint(event); updateFrameSelectionBox(point); const left = Math.min(drag.start.x, point.x), right = Math.max(drag.start.x, point.x), top = Math.min(drag.start.y, point.y), bottom = Math.max(drag.start.y, point.y); const next = event.shiftKey ? new Set(state.selectedFrameIds) : new Set(); $('frameList').querySelectorAll('.frame-card').forEach(card => { const x = card.offsetLeft, y = card.offsetTop, r = x + card.offsetWidth, b = y + card.offsetHeight; if (r >= left && x <= right && b >= top && y <= bottom) next.add(card.dataset.id); }); state.selectedFrameIds = next; drag.box.remove(); state.frameSelectionDrag = null; renderFrames(); }
...
function deleteAnimation(index) { if (state.animations.length <= 1) return toast('至少保留一个动画'); const removed = state.animations.splice(index, 1)[0]; state.activeAnimation = clamp(state.activeAnimation > index ? state.activeAnimation - 1 : state.activeAnimation, 0, state.animations.length - 1); state.activeLayerId = null; state.playIndex = 0; state.motionProgress = 0; state.selectedFrame = -1; state.selectedFrameIds.clear(); renderAnimations(); renderLayers(); renderFrames(); drawPreview(); toast(`已删除${removed.name}`); }
function renderAnimations() { const list = $('animationList'); list.innerHTML = ''; state.animations.forEach((anim, i) => { const row = document.createElement('div'); row.className = `animation-item${i === state.activeAnimation ? ' active' : ''}`; const title = document.createElement('span'); title.textContent = anim.name; const count = document.createElement('small'); count.textContent = `${anim.frameIds.length} Frames`; const remove = document.createElement('button'); remove.type = 'button'; remove.className = 'animation-delete'; remove.hidden = !showLocalFeatures; remove.title = `删除${anim.name}`; remove.textContent = '×'; remove.addEventListener('click', event => { event.stopPropagation(); deleteAnimation(i); }); row.append(title, count, remove); row.addEventListener('click', () => { state.activeAnimation = i; state.activeLayerId = null; state.playIndex = 0; state.motionProgress = 0; state.motionDirection = 1; state.direction = 1; state.selectedFrameIds.clear(); syncSelectionControls(); syncMotionControls(); renderAnimations(); renderLayers(); renderFrames(); drawPreview(); }); list.appendChild(row); }); }
function syncMotionControls() {
  if ($('motionAmount')) $('motionAmount').value = state.motionAmount;
  if ($('motionAmountValue')) $('motionAmountValue').textContent = `${state.motionAmount} px`;
  if ($('motionEnabled')) $('motionEnabled').checked = state.motionEnabled;
...
function updatePreviewMeta() { const frames = activeFrames(); const m = motionConfig(); const motionLabel = m.mode === 'jump' ? `跳跃 ${Math.abs(Number(m.endX) || 0)}px` : m.mode === 'linear' ? `向右 ${Math.abs(Number(m.endX) || 0)}px` : '原地'; $('previewMeta').textContent = frames.length ? `${frames.length} 帧 · ${motionLabel} · ${state.scale}×` : '等待动作'; $('frameCounter').textContent = `第 ${frames.length ? state.playIndex + 1 : 0} / ${frames.length} 帧`; $('previewEmpty').hidden = !!frames.length; $('fpsReadout').textContent = state.fps; }
...
function motionBounds() { const m = motionConfig(); const frames = activeFrames(); if (!frames.length) return { minX: 0, maxX: 0, minY: 0, maxY: 0 }; let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity; const steps = Math.max(24, frames.length * 2); for (let i = 0; i <= steps; i++) { const t = i / steps, p = previewMotionPosition(t), frame = frames[Math.min(frames.length - 1, Math.round(t * (frames.length - 1)))]; minX = Math.min(minX, p.x - frame.anchorX); maxX = Math.max(maxX, p.x + frame.w - frame.anchorX); minY = Math.min(minY, p.y - frame.anchorY); maxY = Math.max(maxY, p.y + frame.h - frame.anchorY); } return { minX, maxX, minY, maxY }; }
...
function previewRenderScale(zoom = state.previewZoom) { return Math.max(1, Math.round(previewFitScale() * zoom)); }
function previewOrigin(z) { const bounds = motionBounds(); return { x: (previewCanvas.width - (bounds.maxX - bounds.minX) * z) / 2 - bounds.minX * z + state.cameraX, y: (previewCanvas.height - (bounds.maxY - bounds.minY) * z) / 2 - bounds.minY * z + state.cameraY }; }
function drawMotionOverlay(z) { const m = motionConfig(); if (m.mode === 'static') return; const origin = previewOrigin(z); const point = (t) => { const p = motionPosition(t, m); return { x: Math.round(origin.x + p.x * z), y: Math.round(origin.y + p.y * z) }; }; const start = point(0), end = point(1); previewCtx.save(); previewCtx.strokeStyle = '#8ed8c4'; previewCtx.lineWidth = 2; previewCtx.setLineDash([6, 5]); previewCtx.beginPath(); for (let i = 0; i <= 24; i++) { const p = point(i / 24); if (i === 0) previewCtx.moveTo(p.x, p.y); else previewCtx.lineTo(p.x, p.y); } previewCtx.stroke(); previewCtx.setLineDash([]); previewCtx.fillStyle = '#f4b765'; [start, end].forEach(p => previewCtx.fillRect(p.x - 6, p.y - 6, 12, 12)); if (m.mode === 'jump') { const control = point(.5); previewCtx.fillStyle = '#f08f83'; previewCtx.fillRect(control.x - 5, control.y - 5, 10, 10); } if (m.mode === 'custom') { const control = { x: Math.round(origin.x + (m.controlX ?? (m.startX + m.endX) / 2) * z), y: Math.round(origin.y + (m.controlY ?? (m.startY + m.endY) / 2) * z) }; previewCtx.fillStyle = '#f08f83'; previewCtx.fillRect(control.x - 5, control.y - 5, 10, 10); } previewCtx.fillStyle = '#fff1d0'; previewCtx.font = '12px ui-monospace,monospace'; previewCtx.fillText('START', start.x + 9, start.y - 9); previewCtx.fillText('END', end.x + 9, end.y - 9); previewCtx.restore(); }
function layerHandleAt(point, z) { const frames = compositionEntries().slice().reverse(); const origin = previewOrigin(z); const travel = previewMotionPosition(previewMotionT()); for (const entry of frames) { if (!entry.layer) continue; const frame = entry.frame; const baseX = origin.x + travel.x * z - frame.anchorX * z + (Number(entry.layer.x) || 0) * z; const baseY = origin.y + travel.y * z - frame.anchorY * z + (Number(entry.layer.y) || 0) * z; const handle = { x: baseX + frame.w * z / 2, y: baseY + frame.h * z / 2 }; if (Math.hypot(point.x - handle.x, point.y - handle.y) < 22) return { layer: entry.layer, frame, baseX, baseY }; } return null; }
function drawLayerOverlay(z) { const entries = compositionEntries().filter(entry => entry.layer); if (!entries.length) return; const origin = previewOrigin(z); const travel = previewMotionPosition(previewMotionT()); previewCtx.save(); previewCtx.font = '11px ui-monospace,monospace'; entries.forEach(({ frame, layer }) => { const baseX = origin.x + travel.x * z - frame.anchorX * z + (Number(layer.x) || 0) * z; const baseY = origin.y + travel.y * z - frame.anchorY * z + (Number(layer.y) || 0) * z; const x = Math.round(baseX + frame.w * z / 2), y = Math.round(baseY + frame.h * z / 2); previewCtx.fillStyle = '#8ed8c4'; previewCtx.fillRect(x - 5, y - 5, 10, 10); previewCtx.fillStyle = '#fff1d0'; previewCtx.fillText(layer.name, x + 9, y - 8); }); previewCtx.restore(); }
...
function drawPreview() { updatePreviewMeta(); previewCtx.clearRect(0, 0, previewCanvas.width, previewCanvas.height); drawPreviewGrid(); const frames = activeFrames(); if (!frames.length) return; const z = previewRenderScale(); const origin = previewOrigin(z); const travel = previewMotionPosition(previewMotionT()); const anchorX = origin.x + travel.x * z; const anchorY = origin.y + travel.y * z; compositionEntries().forEach(({ frame, layer }) => { const offsetX = Number(layer?.x) || 0; const offsetY = Number(layer?.y) || 0; const w = frame.w * z, h = frame.h * z; previewCtx.save(); previewCtx.globalAlpha = layer?.opacity ?? 1; previewCtx.imageSmoothingEnabled = false; previewCtx.drawImage(frame.canvas, Math.round(anchorX - frame.anchorX * z + offsetX * z), Math.round(anchorY - frame.anchorY * z + offsetY * z), w, h); previewCtx.restore(); }); drawLayerOverlay(z); drawMotionOverlay(z); }
function tick(now) { if (state.playing) { const frames = activeFrames(); const current = frames[state.playIndex]; const interval = 1000 / state.fps * (current?.duration || 1) / Math.max(.1, state.previewSpeed); if (now - state.lastTick >= interval) { const len = frames.length; state.playIndex += 1; if (state.playIndex >= len) state.playIndex = state.loop ? 0 : len - 1; state.motionProgress = len > 1 ? state.playIndex / (len - 1) : 1; state.lastTick = now; drawPreview(); if (!state.loop && state.playIndex === len - 1) state.playing = false; } } state.raf = requestAnimationFrame(tick); }
...
function generateAction(mode = 'new') {
  ...
  try {
    ...
    else { state.animations.push({ name: `动作 ${state.animations.length + 1}`, frameIds: ids, layers: defaultLayers() }); state.activeAnimation = state.animations.length - 1; state.activeLayerId = null; sourceLayer = sheetLayer(state.activeSheetId); }
...
function exportMotionPosition(t, anim = activeAnimation()) { const m = anim?.motion || defaultMotion(); let p = clamp(t, 0, 1); if (m.mode !== 'static' && m.loop === 'pingpong') { const phase = p * 2; p = phase <= 1 ? phase : 2 - phase; } const base = motionPosition(p, m); const nudge = simpleMotionOffset(t); return { x: base.x + nudge.x, y: base.y + nudge.y }; }
function exportUnit(samples, scale, bakeMotion = true) { const m = motionConfig(); const paddingPercent = bakeMotion ? (m.padding ?? 10) : 0; let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity; samples.forEach(sample => { const frame = sample.frame || sample; const travel = bakeMotion ? exportMotionPosition(sample.motionT ?? 0) : { x: 0, y: 0 }; minX = Math.min(minX, travel.x - frame.anchorX); maxX = Math.max(maxX, travel.x + frame.w - frame.anchorX); minY = Math.min(minY, travel.y - frame.anchorY); maxY = Math.max(maxY, travel.y + frame.h - frame.anchorY); }); const baseW = Math.max(1, maxX - minX), baseH = Math.max(1, maxY - minY); const padW = Math.ceil(baseW * paddingPercent / 100), padH = Math.ceil(baseH * paddingPercent / 100); minX -= padW; maxX += padW; minY -= padH; maxY += padH; const width = Math.max(2, Math.ceil((maxX - minX) * scale)); const height = Math.max(2, Math.ceil((maxY - minY) * scale)); return { minX, minY, width: width + (width % 2), height: height + (height % 2), bakeMotion }; }
function drawAligned(ctx, sample, unit, x, y, scale) { const frame = sample.frame || sample; if (!frameVisible(frame)) return; ctx.imageSmoothingEnabled = false; const travel = unit.bakeMotion ? exportMotionPosition(sample.motionT ?? 0) : { x: 0, y: 0 }; const m = motionConfig(); const rawX = x + (travel.x - frame.anchorX - unit.minX) * scale; const rawY = y + (travel.y - frame.anchorY - unit.minY) * scale; const dx = m.pixelSnap === false ? rawX : Math.round(rawX); const dy = m.pixelSnap === false ? rawY : Math.round(rawY); ctx.drawImage(frame.canvas, dx, dy, frame.w * scale, frame.h * scale); }
function buildExportCanvas(samples, scale) { const staticSamples = samples.map(sample => ({ frame: sample.frame || sample, motionT: 0 })); const unit = exportUnit(staticSamples, scale, false); const cols = Math.min(staticSamples.length, Math.max(1, Math.floor(32000 / unit.width))); const rows = Math.ceil(staticSamples.length / cols); const sheet = document.createElement('canvas'); sheet.width = cols * unit.width; sheet.height = rows * unit.height; const ctx = sheet.getContext('2d'); ctx.imageSmoothingEnabled = false; staticSamples.forEach((sample, i) => drawAligned(ctx, sample, unit, (i % cols) * unit.width, Math.floor(i / cols) * unit.height, scale)); return sheet; }
...
function exportPlaybackFps() { return clamp(Number(state.fps) * Math.max(.1, Number(state.previewSpeed) || 1), .25, 60); }
function expandedExportFrames() { const source = activeFrames(); const settings = exportSettings(); const samples = []; const loopCount = settings.loops === 0 ? 1 : settings.loops; for (let loop = 0; loop < loopCount; loop++) source.forEach(frame => { for (let i = 0; i < settings.duration * (frame.duration || 1); i++) samples.push({ frame, motionT: 0 }); }); const total = samples.length; samples.forEach((sample, index) => { sample.motionT = total > 1 ? index / (total - 1) : 1; }); return samples; }
...
function currentExportSample() { const frames = activeFrames(); if (!frames.length) return null; const selected = state.selectedFrame >= 0 ? state.frames[state.selectedFrame] : null; const selectedIndex = selected ? frames.indexOf(selected) : -1; const index = selectedIndex >= 0 ? selectedIndex : clamp(state.playIndex, 0, frames.length - 1); return { frame: frames[index], index, motionT: frames.length > 1 ? index / (frames.length - 1) : 1 }; }
...
function frameExportCanvas(frame) { const scale = minimumExportScale(frame); const canvas = document.createElement('canvas'); canvas.width = Math.max(1, frame.w * scale); canvas.height = Math.max(1, frame.h * scale); const ctx = canvas.getContext('2d'); ctx.imageSmoothingEnabled = false; ctx.drawImage(frame.canvas, 0, 0, canvas.width, canvas.height); return canvas; }
function selectedFrameForExport() { const frames = activeFrames(); const selected = state.selectedFrame >= 0 ? state.frames[state.selectedFrame] : null; return selected && frames.includes(selected) ? selected : frames[clamp(state.playIndex, 0, frames.length - 1)]; }
...
async function exportAllFramesPng() { const frames = activeFrames(); if (!frames.length) return toast('当前动作还没有帧'); setTaskStatus('WRITING FILE', '导出全部动作帧', .1); setExportStatus('WRITING FILE', `共 ${frames.length} 帧 · 最小 800×900`, .1); try { for (let i = 0; i < frames.length; i++) { const canvas = frameExportCanvas(frames[i]); downloadBlob(await canvasBlob(canvas), `${exportFilePrefix()}_动作帧_${String(i + 1).padStart(3, '0')}_${canvas.width}x${canvas.height}.png`); setExportStatus('WRITING FILE', `${i + 1} / ${frames.length} 帧`, (i + 1) / frames.length); await new Promise(resolve => setTimeout(resolve, 60)); } setTaskStatus('DONE', '全部动作帧已导出', 1); setExportStatus('DONE', `${frames.length} 张透明 PNG 已下载`, 1); toast('全部动作帧已按像素边缘导出'); } catch (error) { setTaskStatus('ERROR', 'PNG 导出失败', 0); setExportStatus('ERROR', error.message, 0); toast(error.message); } }
...
function deleteSelectedFrames() { const ids = state.selectedFrameIds; if (!ids.size) return toast('请先框选或 Shift 多选帧'); state.frames = state.frames.filter(frame => !ids.has(frame.id)); state.animations.forEach(animation => { animation.frameIds = animation.frameIds.filter(id => !ids.has(id)); animation.layers?.forEach(layer => { if (Array.isArray(layer.frameIds)) layer.frameIds = layer.frameIds.filter(id => !ids.has(id)); }); }); state.selectedFrameIds.clear(); state.selectedFrame = -1; state.playIndex = 0; renderAnimations(); renderLayers(); renderFrames(); drawPreview(); toast(`已删除 ${ids.size} 帧`); }
function duplicateFrame() { const f = state.frames[state.selectedFrame]; if (!f) return toast('请先选择一帧'); const copy = { ...f, id: crypto.randomUUID(), canvas: f.canvas, dataUrl: f.dataUrl }; state.frames.push(copy); const ids = state.animations[state.activeAnimation].frameIds; ids.splice(ids.indexOf(f.id) + 1, 0, copy.id); state.selectedFrame = state.frames.length - 1; renderFrames(); renderAnimations(); drawPreview(); }
...
$('modeSwitch').addEventListener('click', e => { const button = e.target.closest('button[data-mode]'); if (!button) return; state.mode = button.dataset.mode; syncMode(); });
 $('fps').addEventListener('input', e => { state.fps = clamp(Number(e.target.value) || 12, 1, 60); $('fpsReadout').textContent = state.fps; }); $('previewSpeed').addEventListener('change', e => { state.previewSpeed = Number(e.target.value) || .5; drawPreview(); }); $('scale').addEventListener('change', e => { state.scale = Number(e.target.value); drawPreview(); }); $('motionEnabled').addEventListener('change', e => { state.motionEnabled = e.target.checked; drawPreview(); }); $('motionAmount').addEventListener('input', e => { state.motionAmount = clamp(Number(e.target.value) || 0, 0, 6); syncMotionControls(); drawPreview(); }); $('motionModeSimple').addEventListener('change', e => { const m = motionConfig(); m.mode = e.target.value; m.loop = m.mode === 'static' ? 'static' : 'once'; m.startX = 0; m.startY = 0; m.endY = 0; if (m.mode === 'static') { m.endX = 0; m.height = 0; } else { if (!Number(m.endX)) m.endX = 96; if (m.mode === 'jump' && !Number(m.height) && activeFrames().length) m.height = Math.max(1, 120 / averageCharacterHeight()); } syncMotionControls(); drawPreview(); }); $('motionDistance').addEventListener('input', e => { const m = motionConfig(); m.endX = clamp(Number(e.target.value) || 0, 0, 320); if (m.mode === 'static' && m.endX > 0) m.mode = 'linear'; syncMotionControls(); drawPreview(); }); $('motionHeight').addEventListener('input', e => { const m = motionConfig(); m.height = clamp(Number(e.target.value) || 0, 0, 240) / Math.max(1, averageCharacterHeight()); syncMotionControls(); drawPreview(); }); $('pickBackground')?.addEventListener('click', () => { state.backgroundPicking = true; sheetCanvas.style.cursor = 'crosshair'; toast('请在 Sprite Sheet 上点击一个背景像素'); }); $('backgroundViewMode')?.addEventListener('change', e => { state.backgroundView = e.target.value; drawSheet(); }); $('panelRatio').addEventListener('input', e => { state.panelRatio = clamp(Number(e.target.value) || 75, 40, 80); resizeWorkspacePanels(); }); $('frameDuration')?.addEventListener('change', e => { const f = state.frames[state.selectedFrame]; if (f) { f.duration = Number(e.target.value); drawPreview(); } }); $('playPause').addEventListener('click', () => { if (!activeFrames().length) return toast('请先生成或选择一个动作'); state.playing = !state.playing; state.lastTick = performance.now(); $('playPause').textContent = state.playing ? 'Ⅱ' : '▶'; }); $('prevFrame').addEventListener('click', () => { const len = activeFrames().length; if (len) { state.playing = false; state.playIndex = (state.playIndex - 1 + len) % len; $('playPause').textContent = '▶'; drawPreview(); } }); $('nextFrame').addEventListener('click', () => { const len = activeFrames().length; if (len) { state.playing = false; state.playIndex = (state.playIndex + 1) % len; $('playPause').textContent = '▶'; drawPreview(); } });
 $('sheetZoomIn').addEventListener('click', () => { state.sheetZoom = clamp(state.sheetZoom * 1.5, .25, 6); syncSheetZoom(); }); $('sheetZoomOut').addEventListener('click', () => { state.sheetZoom = clamp(state.sheetZoom / 1.5, .25, 6); syncSheetZoom(); }); $('sheetZoomReset').addEventListener('click', () => { state.sheetZoom = 1; syncSheetZoom(); }); $('previewResetView').addEventListener('click', () => { state.previewZoom = 1; state.cameraX = 0; state.cameraY = 0; $('previewZoomLabel').textContent = '100%'; drawPreview(); }); $('previewZoomIn').addEventListener('click', () => { state.previewZoom = clamp(state.previewZoom * 1.5, .25, 12); $('previewZoomLabel').textContent = `${Math.round(state.previewZoom * 100)}%`; drawPreview(); }); $('previewZoomOut').addEventListener('click', () => { state.previewZoom = clamp(state.previewZoom / 1.5, .25, 12); $('previewZoomLabel').textContent = `${Math.round(state.previewZoom * 100)}%`; drawPreview(); }); $('newAnimation').addEventListener('click', newAnimation); $('deleteSelectedFrames')?.addEventListener('click', deleteSelectedFrames); $('addLayer').addEventListener('click', () => { const layers = ensureLayers(); layers.push({ id: `layer-${Date.now()}`, name: `新图层 ${layers.length + 1}`, mode: 'sequence', visible: true, opacity: 1, x: 0, y: 0 }); renderLayers(); renderFrames(); toast('已添加图层'); }); $('exportMenu').addEventListener('click', exportMenu); $('exportCurrentFrame').addEventListener('click', () => { $('exportPopover').classList.remove('open'); exportCurrentFramePng(); }); $('exportAllFrames').addEventListener('click', () => { $('exportPopover').classList.remove('open'); exportAllFramesPng(); }); $('exportSequence').addEventListener('click', () => { $('exportPopover').classList.remove('open'); exportSequence(); }); $('exportLocalGif').addEventListener('click', () => { $('exportPopover').classList.remove('open'); exportLocalGif(); }); $('exportLocalMov').addEventListener('click', () => { $('exportPopover').classList.remove('open'); exportLocalMov(); }); $('exportDiagnostics').addEventListener('click', () => { $('exportPopover').classList.remove('open'); exportDiagnostics(); });
...
function motionHandleAt(point, z) { const m = motionConfig(); if (m.mode === 'static') return null; const origin = previewOrigin(z); const toScreen = p => ({ x: origin.x + p.x * z, y: origin.y + p.y * z }); const start = toScreen({ x: m.startX, y: m.startY }); const end = toScreen({ x: m.endX, y: m.endY }); if (Math.hypot(point.x - start.x, point.y - start.y) < 18) return 'start'; if (Math.hypot(point.x - end.x, point.y - end.y) < 18) return 'end'; if (m.mode === 'jump') { const h = toScreen({ x: (m.startX + m.endX) / 2, y: (m.startY + m.endY) / 2 - motionPixelHeight(m) }); if (Math.hypot(point.x - h.x, point.y - h.y) < 18) return 'height'; } if (m.mode === 'custom') { const c = toScreen({ x: m.controlX ?? (m.startX + m.endX) / 2, y: m.controlY ?? (m.startY + m.endY) / 2 }); if (Math.hypot(point.x - c.x, point.y - c.y) < 18) return 'control'; } return null; }
previewCanvas.addEventListener('pointerdown', event => { if (!activeFrames().length) return; previewCanvas.setPointerCapture(event.pointerId); const p = previewPoint(event); const z = previewRenderScale(); const layerHandle = layerHandleAt(p, z); if (layerHandle) { state.layerDrag = layerHandle; previewCanvas.style.cursor = 'move'; return; } const handle = motionHandleAt(p, z); if (handle) { state.motionDrag = handle; previewCanvas.style.cursor = 'crosshair'; return; } state.draggingPreview = true; state.dragStart = { x: p.x, y: p.y, cameraX: state.cameraX, cameraY: state.cameraY }; previewCanvas.style.cursor = 'grabbing'; });
previewCanvas.addEventListener('pointermove', event => { const p = previewPoint(event); if (state.layerDrag) { const z = previewRenderScale(); const origin = previewOrigin(z); const travel = previewMotionPosition(previewMotionT()); state.layerDrag.layer.x = Math.round((p.x - (origin.x + travel.x * z - state.layerDrag.frame.anchorX * z)) / z); state.layerDrag.layer.y = Math.round((p.y - (origin.y + travel.y * z - state.layerDrag.frame.anchorY * z)) / z); renderLayers(); drawPreview(); return; } if (state.motionDrag) { const m = motionConfig(); const z = previewRenderScale(); const origin = previewOrigin(z); const sx = Math.round((p.x - origin.x) / z), sy = Math.round((p.y - origin.y) / z); if (state.motionDrag === 'start') { m.startX = sx; m.startY = sy; } else if (state.motionDrag === 'end') { m.endX = sx; m.endY = sy; } else if (state.motionDrag === 'height') { m.height = Math.max(0, Math.round(((m.startY + m.endY) / 2 - sy) / averageCharacterHeight() * 2) / 2); } else if (state.motionDrag === 'control') { m.controlX = sx; m.controlY = sy; } syncMotionControls(); drawPreview(); return; } if (!state.draggingPreview || !state.dragStart) return; state.cameraX = state.dragStart.cameraX + p.x - state.dragStart.x; state.cameraY = state.dragStart.cameraY + p.y - state.dragStart.y; drawPreview(); });
const endPreviewDrag = () => { state.draggingPreview = false; state.motionDrag = null; state.layerDrag = null; state.dragStart = null; previewCanvas.style.cursor = 'grab'; };
