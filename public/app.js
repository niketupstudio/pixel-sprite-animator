...
function exportEntriesAt(frameIndex, layers = selectedLayers()) { const entries = []; layers.forEach(layer => { const frame = layerFrameAt(layer, frameIndex); if (frame) entries.push({ frame, layer }); }); if (!entries.length && !layers.length) { const frames = activeFrames(); const frame = frames[frameIndex % Math.max(1, frames.length)]; if (frame) entries.push({ frame, layer: null }); } return entries; }
function exportLayerFrameCount(layers) { return layers.length ? layers.reduce((max, layer) => Math.max(max, layerFrames(layer).length), 0) : activeFrames().length; }
function expandedExportFrames() { const layers = selectedLayers(); const source = exportLayerFrameCount(layers); const settings = exportSettings(); const samples = []; const loopCount = settings.loops === 0 ? 1 : settings.loops; for (let loop = 0; loop < loopCount; loop++) for (let index = 0; index < source; index++) { const entries = exportEntriesAt(index, layers); if (!entries.length) continue; const frame = entries[0]?.frame; for (let i = 0; i < settings.duration * (frame?.duration || 1); i++) samples.push({ entries, frame, motionT: 0 }); } const total = samples.length; samples.forEach((sample, index) => { sample.motionT = total > 1 ? index / (total - 1) : 1; }); return samples; }
...
$('fps')?.addEventListener('input', event => { const layer = activeLayer(); if (layer) layer.fps = clamp(Number(event.target.value) || 12, 1, 60); });
 ...
 $('playPause')?.addEventListener('click', event => { event.preventDefault(); event.stopImmediatePropagation(); if (!previewFrames().length) return toast('请先生成或选择一个动作'); if (state.playing) { state.playing = false; $('playPause').textContent = '▶'; } else if (motionConfig().loop === 'once' && state.playIndex >= previewFrames().length - 1) restartPreviewPlayback(); else { state.playing = true; state.lastTick = performance.now(); $('playPause').textContent = 'Ⅱ'; } }, true);
